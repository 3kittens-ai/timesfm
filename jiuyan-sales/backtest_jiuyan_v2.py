...
def fetch_daily_sales(db_path: str, sku: str, end_date: str, days: int = 90) -> np.ndarray:
...
def load_backtest_data(
    db_path: str, cutoff_date: str, horizon: int, min_context_months: int = 12, max_skus: int | None = None, daily_days: int = 400
) -> tuple[list[np.ndarray], list[np.ndarray], list[str], list[np.ndarray]]:
    """
    加载回测数据。
    返回:
      inputs: 历史输入数据 (月度，不含截止月如果 cutoff 是月中)
      actuals: 真实对齐的未来数据 (从 cutoff_date 之后第一个完整月开始，或者包含部分月)
      sku_index: SKU 列表
      daily_contexts: 截止日期前的日销量背景 (长度为 daily_days)
    """
    conn = sqlite3.connect(db_path)
    ...
    for sku, group in df.groupby("variant_key"):
        ...
        # 拆分历史和未来
        # 如果是月中（day > 1），则月度 context 只到上个月底
        is_mid_month = cutoff_ts.day > 1
        adj_cutoff_ts = cutoff_ts - pd.offsets.MonthEnd(1) if is_mid_month else cutoff_ts
        
        history_df = merged[merged["date"] <= adj_cutoff_ts]
        # future_df 必须严格从历史截止月的下个自然月开始，防止出现重叠 (训练集泄露或错位)
        forecast_start = adj_cutoff_ts + pd.offsets.MonthBegin(1)
        future_df = merged[merged["date"] >= forecast_start]
    ...
    for sku in sku_index:
        daily_contexts.append(fetch_daily_sales(db_path, str(sku), cutoff_date, daily_days))

    return inputs, actuals, sku_index, daily_contexts
...
def main():
    ...
    parser.add_argument("--refine", action="store_true", help="启用日度补全修正 (针对月中 cutoff)")
    parser.add_argument("--daily-days", type=int, default=400, help="用于日度修正的背景天数 (默认 400，覆盖 1 年以上以获取季节性)")
    ...
    inputs, actuals, sku_index, daily_contexts = load_backtest_data(
        DB_PATH, args.cutoff, args.horizon, min_context_months=12, max_skus=max_skus_arg, daily_days=args.daily_days
    )
    
    # 手动过滤指定 SKU
    if target_skus:
        filtered_inputs, filtered_actuals, filtered_sku_index, filtered_daily = [], [], [], []
        ...
        for i, sku in enumerate(sku_index):
            ...
            if clean_sku in target_set or str(sku) in target_set:
                ...
                filtered_daily.append(daily_contexts[i])
        inputs, actuals, sku_index, daily_contexts = filtered_inputs, filtered_actuals, filtered_sku_index, filtered_daily
    ...
    if args.refine and cutoff_ts.day > 1:
        print(f"\n✨ 正在执行日度修正 (Refinement)...")
        first_month_days = calendar.monthrange(cutoff_ts.year, cutoff_ts.month)[1]
        remaining_days = first_month_days - cutoff_ts.day
        
        if remaining_days > 0:
            # 批量预测当前月剩余天数
            # 注意: daily_contexts 每个输入都是 90 天
            daily_point, _ = model.forecast(horizon=remaining_days, inputs=daily_contexts)
            
            for i in range(len(sku_index)):
                # 已发生的销量 (1号到截止日期)
                actual_so_far = np.sum(daily_contexts[i][-cutoff_ts.day:]) 
                # 预测的剩余销量
                ...
                if i < 3:
                     print(f"   SKU {sku_index[i]}: 第一月预测修正为 {refined_total:.1f} (实发:{actual_so_far:.0f} + 预测:{predicted_remaining:.1f})")
        else:
            # 如果刚好是月末，直接用已发生销量
            for i in range(len(sku_index)):
                point[i][0] = np.sum(daily_contexts[i][-cutoff_ts.day:])
